...
plot_ly(countypop %>% filter(YEAR == 2019), x = ~POPESTIMATE, y = ~CTYNAME, type = 'bar', color = ~CTYNAME, colors = county_color, orientation = 'h') %>%
  layout(title= '<b> Estimated County Population by Year </b>',
         xaxis= list(title='Population', tickformat=',', range=c(0,1250000)),
         yaxis= list(title=''),
...
## Unemployment
```{r}
plot_ly(unemployment, x=~Date, y=~Unemployment*100, color=~County, colors=county_color, type='scatter', mode='lines') %>%
  layout(title='<b> County Population Percentage by Place of Birth </b>',
         legend=list(title=list(text='<b> Unemployment Rate over Time </b>')),
         xaxis= list(title='Date'),
         yaxis= list(title='Unemployment %', ticksuffix='%', range=c(0,25)))

plot_ly(unemployment %>% filter(Year==2015, Month==6), x=~Unemployment*100, y=~County,
...
plot_ly(income %>% filter(Year == 2014), y=~County, x=~(Numerator_value/Total)*100, color=~Measure, type='bar') %>%
  layout(barmode = 'stack',
...
plot_ly(education %>% group_by(Year, Measure, County) %>%
  summarise(Numerator = sum(Numerator_value),
            Denominator = mean(Total)) %>% filter(Year == 2018, Measure == 'High School Diploma'), y=~County, color=~County, colors=county_color, x=~(Numerator/Denominator)*100, type='bar') %>%
  layout(title='<b> Educational Attainment % by Attainment Group </b>',
         yaxis=list(title='County'),
         xaxis=list(title='Percentage of Population', ticksuffix='%', range=c(0,50)))
```
...
## Drug-Related Deaths
```{r}
plot_ly(drug %>% filter(Year==2017), x=~Crude_Rate, y=~County, type='bar', colors=~county_color, color=~County) %>%
...
## STD Cases
```{r, warning=FALSE}
plot_ly(std %>% filter(Year==2019),
        x=~Cases, y=~County, color=~STD,
        type='bar') %>%
  layout(title= '<b> STD Cases by STD </b>',
         xaxis= list(title='Cases', tickformat=',', range=c(0,15000)),
         yaxis= list(title=''))
plot_ly(std,
  ...
  layout(title= '<b> STD Cases Over Time </b>',
         xaxis= list(title='Year'),
         yaxis= list(title='Cases', tickformat=',', range=c(0,15000)))
```
...
# Rate per 100,000
plot_ly(births %>% filter(Year==2019, Measure=='Birth_Rate'),
        y=~County, x=~Value, color=~County, colors=county_color, type='bar') %>%
  layout(title= '<b> Births per 100,000 People </b>',
         yaxis= list(title='Counties'),
         xaxis= list(title='Births', tickformat=',', range=c(0,2000)))

plot_ly(births %>% filter(Measure=='Birth_Rate'),
        y=~Value, x=~Year, color=~County, colors=county_color, type='scatter', mode='lines') %>%
  layout(title= '<b> Births Over Time </b>',
         xaxis= list(title='Year'),
         yaxis= list(title='Births', tickformat=',', range=c(800,2000)))
```
...
plot_ly(housing %>% filter(Year == 2014),
  ...
  layout(title= '<b> Median House Age </b>',
         yaxis= list(title='Counties'),
         xaxis= list(title='House Age', tickformat=',', range=c(0,100)))
```
...
plot_ly(poverty %>% filter(Year==2014),
        ...
        color=~County, colors=county_color, type='bar') %>%
      layout(xaxis=list(title='Poverty Rate', ticksuffix='%', range=list(0,30)))
